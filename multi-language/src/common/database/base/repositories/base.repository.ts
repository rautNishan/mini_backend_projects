...
export abstract class BaseRepository<T extends DatabaseBaseEntity> {
    ...
    if (options?.defaultSortColumn) {
      find.order = {
        [options.defaultSortColumn]:
          options.sortOrder ?? options.defaultSortOrder ?? SORT_ORDER_ENUM.DESC,
      };
    }
  ...
  async _paginatedQueryBuilderByLimit(
    options: IPaginateQueryBuilderOption,
  ): Promise<Result> {
    const {
      page = 1,
  ...
  async _softDelete(repo: T, options?: IUpdateOptions<T>): Promise<T> {
    if (options?.entityManager) {
      await options.entityManager.softDelete(this._entityRepo.target, repo.id);
    } else {
      await this._entityRepo.softDelete(repo.id);
    }
    return repo;
  }
