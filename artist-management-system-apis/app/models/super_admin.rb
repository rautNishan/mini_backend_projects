class SuperAdmin < ApplicationRecord
  ...
  message: "only allows letters" }
  validates :email, presence: true, length: { maximum: 255 }, format: { with:  /\A[a-z0-9.!\#$%&'*+\/=?^_`{|}~-]+@[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*\z/, message: "must be lowercase and valid" }, uniqueness: { case_sensitive: false }
  has_secure_password
